...
namespace LoLTournament.Authentication
{
    public class MongoMembershipProvider : MembershipProvider
    {
        ...
        {
            return CreateUser(username, password, email, out status).ToMembershipUser();
        }
        ...
        public User CreateUser(string username, string password, string email, out MembershipCreateStatus status)
        {
            ...
            var user = new User(username, password, email, "", "", true, false, DateTime.Now, DateTime.Now, DateTime.Now, DateTime.Now, DateTime.Now, new List<string>());
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            return Mongo.Users.FindAll().Any(x => x.UserName == username && Crypto.VerifyHashedPassword(x.Password, password));
        }
        ...
        {
            var user = Mongo.Users.Find(Query<User>.Where(x => x.Id == (ObjectId) providerUserKey)).
                SingleOrDefault();
        ...
        {
            var user = Mongo.Users.Find(Query<User>.Where(x => x.UserName == username)).
                SingleOrDefault();
        ...
        {
            var u = Mongo.Users.Find(Query<User>.Where(x => x.Email == email)).SingleOrDefault();
